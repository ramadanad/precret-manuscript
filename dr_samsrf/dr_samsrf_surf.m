...
elseif strcmpi(Type, 'Sigma') || strcmpi(Type, 'Fwhm') || strcmpi(Type, 'Visual Area') || strcmpi(Type, 'Spread') ...
    ...
    %% DR: Start of change
    % This is to use the full 256-entry colormap for all maps.
    % Convert to integers (use full 256-entry colormap)
    mainN = 256;

    
    % Colormap
    if strcmpi(Type, 'R^2') || strcmpi(Type, 'nR^2')
        % Normalize 0..1 then map to 1..mainN (MATLAB 1-based)
        Pha = round( (X - Thrsh(2)) / AdjThr * (mainN - 1) ) + 1;
        Cmap = [samsrf_cmap(SamSrfDefs.def_cmap_r2, mainN); CurvGrey];
        % Rubbish/NaN -> 0
        Pha(r|isnan(Pha)|isinf(Pha)) = 0;
        Pha = round(Pha);
        Pha(Pha < 1) = 1;
        Pha(Pha > size(Cmap,1)) = size(Cmap,1);
    elseif strcmpi(Type, 'BensonEcc')
        % use same colormap as for eccentricity
    ...
    elseif strcmpi(Type, 'BensonPol')
        % Polar map
        Pha = -(Srf.Data(2,:)-90); % Benson polar angle is defined differently, so we need to adjust it;
    ...
    elseif strcmpi(Type, 'Volume #1')
        Pha = round(X * 2.56); % to ge the range 0 to 256
        ...
        % this part is left as it was before
        % Convert to integers
